...
-- Verificar conductores sin id de empleado
SELECT ID_EMPLEADO 
FROM CONDUCTOR 
WHERE ID_EMPLEADO NOT IN (SELECT ID_EMPLEADO FROM EMPLEADO);

-- Verificar vehículos sin GPS asignado
SELECT ID_VEHICULO 
FROM VEHICULO 
WHERE ID_GPS NOT IN (SELECT ID_GPS FROM GPS);

-- Verificar vehículos sin conductores asignados
SELECT ID_VEHICULO, ID_EMPLEADO 
FROM VEHICULO 
WHERE ID_EMPLEADO IS NOT NULL 
AND ID_EMPLEADO NOT IN (SELECT ID_EMPLEADO FROM EMPLEADO);

-- Verificar conductores con horas_semana negativas
SELECT ID_EMPLEADO, HORAS_SEMANA 
FROM CONDUCTOR 
WHERE HORAS_SEMANA < 0;

-- Verificar vehículos con capacidad negativa
SELECT ID_VEHICULO, CAPACIDAD_KG 
FROM VEHICULO 
WHERE CAPACIDAD_KG < 0;

-- Verificar temperaturas de los refrigerados
SELECT ID_VEHICULO, TEMP_MIN, TEMP_MAX 
FROM VEHICULO_REFRIGERADO 
WHERE TEMP_MAX <= TEMP_MIN;

-- Verificar rutas con distancias imposibles
SELECT ID_RUTA, DISTANCIA_KM 
FROM RUTA 
WHERE DISTANCIA_KM <= 0;

-- Verificar envíos con estado inválido
SELECT ID_ENVIO, ESTADO 
FROM ENVIO 
WHERE ESTADO NOT IN ('Entregado','Pendiente','En camino');

-- Verificar vehículos ADR con clase de mercancía inválida
SELECT ID_VEHICULO, CLASEMERCANCIA 
FROM VEHICULO_ADR 
WHERE CLASEMERCANCIA NOT IN ('1','2','3','4.1','4.2','4.3','5.1','5.2','6.1','6.2','7','8','9');

-- Verificar paquetes con número o peso negativo
SELECT N_PAQUETE, ID_ENVIO, PESO 
FROM PAQUETE 
WHERE N_PAQUETE < 0 OR PESO < 0;

-- Verificar matrículas duplicadas en vehiculo
...
-- Verificar dni duplicados en persona
SELECT DNI, COUNT(*) 
FROM PERSONA 
GROUP BY DNI 
HAVING COUNT(*) > 1;

-- Verificar empleados que son tanto conductores como mecánicos
SELECT ID_EMPLEADO 
FROM CONDUCTOR 
WHERE ID_EMPLEADO IN (SELECT ID_EMPLEADO FROM MECANICO);

-- Verificar empleados que son tanto conductores como administrativos
SELECT ID_EMPLEADO 
FROM CONDUCTOR 
WHERE ID_EMPLEADO IN (SELECT ID_EMPLEADO FROM ADMINISTRATIVO);

-- Verificar supervisores que se supervisan a sí mismos
SELECT ID_EMPLEADO_SUPERVISOR, ID_EMPLEADO_SUPERVISADO 
FROM SUPERVISA 
WHERE ID_EMPLEADO_SUPERVISOR = ID_EMPLEADO_SUPERVISADO;

-- Verificar clientes que son tanto persona como empresa
SELECT ID_CLIENTE 
FROM PERSONA 
WHERE ID_CLIENTE IN (SELECT ID_CLIENTE FROM EMPRESA);

-- Verificar ejecuciones sin conductores asignados
SELECT ID_EJECUCION 
FROM EJECUCION 
WHERE ID_EJECUCION NOT IN (SELECT ID_EJECUCION FROM EJECUCION_CONDUCTOR);

-- Verificar paradas sin ruta existente
SELECT N_PARADA, ID_RUTA 
FROM PARADA 
WHERE ID_RUTA NOT IN (SELECT ID_RUTA FROM RUTA);

-- Verificar paquetes sin envío asociado
SELECT N_PAQUETE, ID_ENVIO 
FROM PAQUETE 
WHERE ID_ENVIO NOT IN (SELECT ID_ENVIO FROM ENVIO);

-- Verificar empleados sin información básica
SELECT ID_EMPLEADO 
FROM EMPLEADO 
WHERE TELEFONO IS NULL OR NOMBRE_COMPLETO IS NULL OR NSS IS NULL OR DNI IS NULL;

-- Verificar clientes sin información completa
SELECT ID_CLIENTE 
FROM CLIENTE 
WHERE ID_CLIENTE NOT IN (SELECT ID_CLIENTE FROM PERSONA)
AND ID_CLIENTE NOT IN (SELECT ID_CLIENTE FROM EMPRESA);

-- Verificar GPS sin información completa
SELECT ID_GPS 
FROM GPS 
WHERE MODELO IS NULL OR NUM_SERIE IS NULL;

-- Verificar si hay supervisores que no existen en la tabla empleado
SELECT DISTINCT ID_EMPLEADO_SUPERVISOR 
FROM SUPERVISA 
WHERE ID_EMPLEADO_SUPERVISOR NOT IN (SELECT ID_EMPLEADO FROM EMPLEADO);

-- Verificar empleados sin tipo
SELECT ID_EMPLEADO 
FROM EMPLEADO 
WHERE ID_EMPLEADO NOT IN (SELECT ID_EMPLEADO FROM CONDUCTOR)
AND ID_EMPLEADO NOT IN (SELECT ID_EMPLEADO FROM MECANICO)
AND ID_EMPLEADO NOT IN (SELECT ID_EMPLEADO FROM ADMINISTRATIVO);
