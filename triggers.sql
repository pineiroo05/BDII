...
BEGIN
    ...
    AND NOT (ID_ENVIO = :NEW.ID_ENVIO AND N_PAQUETE = :NEW.N_PAQUETE); -- Excluye la fila actual si es UPDATE
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        -- Si no encuentra el vehículo o la ejecución, no puede validar (pero la FK debería prevenirlo)
...
FOR EACH ROW
BEGIN
    -- El CHECK en la tabla ya tiene >= 0, este trigger añade el límite superior de la semántica.
...
DECLARE
    ...
    v_id_envio_temp ENVIO.ID_ENVIO%TYPE := 'ENV_TMP';
...
BEGIN
    ...
    DECLARE
        v_fecha_check DATE;
        v_fecha_fija DATE := TO_DATE('2023-01-01', 'YYYY-MM-DD');
